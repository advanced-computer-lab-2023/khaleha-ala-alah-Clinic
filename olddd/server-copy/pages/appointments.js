...
window.onload = function () {
  fetch(`http://localhost:4000/patients/getappointments`)
    .then((response) => {
      if (!response.ok) {
    ...
    .then((data) => {
      ...
      fetch("http://localhost:4000/patients/patientdoctors")
        .then((response2) => {
          if (!response2.ok) {
            throw new Error("Network response was not ok");
          }
        ...
        .then((data2) => {
          ...
          for (let i = 0; i < data.appointments.length; i++) {
            ...
            const doctorInfo = data2.doctors[i];
...
function displayAppointments(appointments) {
  ...
  for (let i = 0; i < appointments.length; i++) {
    ...
    const doctorInfo = doctors.doctors[i];
