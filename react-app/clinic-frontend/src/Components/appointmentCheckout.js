...
const amount22  = doctor.hourlyRate;
  const onToken = (token) => {
    ...
    .then(async data => {
      ...
      try {
        console.log(`http://localhost:4000/patients/SelectAppointment/${doctor.userID}/${date.toISOString()}`);
        const response = await fetch(
          `http://localhost:4000/patients/SelectAppointment/${doctor.userID}/${date.toISOString()}`,
          {
            method: "POST",
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
