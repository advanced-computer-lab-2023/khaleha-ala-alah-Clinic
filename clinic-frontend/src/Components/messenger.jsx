...
import HeaderDoc from "../Elements/HeaderDoctor";
import NavBarDoc from "../Elements/NavBarDoctor";
import HeaderPat from "../Elements/Header";
import NavBarPat from "../Elements/NavBar";
  ...
  useEffect(() => {
    ...
    if (conversations?.members.includes(arrivalMessage.sender)) {
      setTimeout(() => {
        axios
          .post(
            `http://localhost:4000/conversations/deleteMessagesNotification`,
    ...
    const getMessages = async () => {
      try {
        const res = await axios.get(
          `http://localhost:4000/messages/${conversations._id}`
        );
